...
//    }
    ...
    ROS_INFO("=== CARGA DE MAPA ===");
    ROS_INFO("Ruta del mapa: %s", mapPath.string().c_str());
    ROS_INFO("Tamaño de imagen cargada: %d x %d pixeles", mapImg.cols, mapImg.rows);
    // ROS_INFO("Resolución: %f metros/píxel", metersPerPixel);
    // ROS_INFO("Ancho en metros: %f", mapImg.cols * metersPerPixel);
    // ROS_INFO("Alto en metros: %f", mapImg.rows * metersPerPixel);
