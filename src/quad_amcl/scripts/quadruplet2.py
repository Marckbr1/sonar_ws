    ...
    def process(self):
        ...
        for j, sat_vec in enumerate(self.sat_out):
            ...
            if distance < best_distance:
                ...
                best_idx = j
        try:
            sonar_vis = (self.son_ready_image * 255).astype(np.uint8)
            satelite_vis = (self.satellite[best_idx] * 255).astype(np.uint8)

            # Convertir RGB a BGR para OpenCV si se quiere guardar directamente
            sonar_vis = cv2.cvtColor(sonar_vis, cv2.COLOR_RGB2BGR)
            satelite_vis = cv2.cvtColor(satelite_vis, cv2.COLOR_RGB2BGR)

            combined = np.hstack((sonar_vis, satelite_vis))
            cv2.imwrite("/scripts/best_match.png", combined)
            rospy.loginfo("Saved comparison image at /scripts/best_match.png")
        except Exception as e:
            rospy.logwarn(f"Could not save comparison image: {e}")
